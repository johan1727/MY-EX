...
export default function ActiveCallScreen() {
    ...
    const [statusText, setStatusText] = useState('Mant├®n presionado para hablar');
    ...
    const loadProfileData = async () => {
        try {
            ...
            if (!data) {
                ...
                setSystemPrompt(`
                    Act├║a como mi Ex pareja.
                    Tu nombre es ${name || 'Ex'}.
                    Personalidad: Sarc├ística, directa pero con momentos de vulnerabilidad.
                `);
                return;
            }
            ...
            const baseMap: Record<string, string> = {
                'ex': 'Act├║a como mi Ex pareja.',
                'partner': 'Act├║a como mi Pareja actual.',
                'crush': 'Act├║a como mi Crush (inter├®s rom├íntico).',
                'friend': 'Act├║a como mi mejor amigo/a.',
                'family': 'Act├║a como un familiar cercano.',
                'fallecido': 'Act├║a como una persona fallecida muy querida (simulaci├│n de memoria).'
            };
            ...
            const personality = data.master_prompt
                ...
                : 'Personalidad: Sarc├ística y directa.';
            ...
            setSystemPrompt(`
                ...
                CONTEXTO: Est├ís en una LLAMADA TELEF├ôNICA con el usuario.
                INSTRUCCIONES CLAVE:
                1. TUS RESPUESTAS DEBEN SER CORTAS (M├íximo 2 o 3 oraciones). Es una charla hablada, no un email.
                2. Act├║a natural, usa muletillas si cuadra con la personalidad ("este...", "o sea").
                3. Reacciona al tono de voz del usuario (si suena triste, s├® emp├ítico/a; si suena feliz, s├¡guele el rollo).
            `);

        } catch (e) {
            console.error('Error constructing prompt', e);
            // Emergency Fallback
            setSystemPrompt(`Act├║a como mi Ex pareja. Tu nombre es ${name}.`);
        }
    };
    ...
    const checkLimits = async () => {
        ...
        if (!status.canCall) {
            ...
            if (Platform.OS !== 'web') Alert.alert('L├¡mite Alcanzado', 'Has usado todos tus minutos.');
        ...
        // Warnings
        if (status.usagePercent >= 90) setStatusText('ÔÜá´©Å ALERTA: 90% de uso. Te queda poco.');
    ...
    const processAudioResponse = async (audioUri: string) => {
        try {
            ...
            // Contexto m├¡nimo para no gastar tokens
            const finalPrompt = `
                ${systemPrompt || 'Act├║a como mi Ex.'}
                
                HISTORIAL RECIENTE: ${conversationHistory.current.join(' | ')}
                
                INSTRUCCI├ôN: Usuario te dice algo (en el audio adjunto). RESP├ôNDELE CORTANTE Y REALISTA (Max 2 frases).
            `;
            ...
            setStatusText('Error de conexi├│n ÔØî');
            setCallState('idle');
